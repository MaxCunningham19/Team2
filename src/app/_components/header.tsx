    ...
    <header className="container fixed left-1/2 top-0 z-[5] -translate-x-1/2 transform px-4 pt-8">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="w-full max-w-xl">
          <nav className="flex items-center justify-between rounded-full border-2 border-[#745543]/20 bg-[#e6ccc0]/50 px-6 py-2 backdrop-blur-md">
            <div className="relative flex items-center gap-4">
              {navItems.map((item, index) => (
                <div key={item.title} className="flex items-center">
                  <Link
                    href={item.href}
                    className={`${pathname === item.href ? "text-[#a88a7c]" : ""} relative px-2 py-1 text-[#745543] transition-colors
                      hover:text-[#a88a7c]`}
                  >
                    {item.title}
                  </Link>
